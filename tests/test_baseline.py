...
def fake_output_machine(t, beam_size):
    ...
    if t >= 0:
        ...
        indices = [1, 2] +[0 for _ in range(beam_size-2)]
...
class BaselineRecomb(unittest.TestCase):
    ...
    def test_recomb_baseline(self):
        recomb_baseline(doc_input_ids=None, param_sim_function=self.param_sim_function,  eos_token_id = 10, model=fake_output_machine, debug=True)
