...
def greedy_generate_sequence(doc_input_ids, model, start_seed, device, max_len=20, extra_steps=7):
    ...
    return pointer, fish_bone_teacher, fish_bone_student, call
...
def rollout_recombine(doc_input_ids, model, seed:BeamState, teacher_seed:BeamState,teacher_final_seed,  extra_steps=10, max_len=20):
    ...
    roll_out_state, fb_teacher, fb_student, ncall = greedy_generate_sequence(doc_input_ids, model,
    ...
    return flag_recomb, output, ncall
    
def random_teacher(max_len, eos_token_id, pad_token_id) -> BeamState:
    cnt = 0
...
def best_first_search(doc_input_ids, model,  pad_token_id=0, eos_token_id=21, max_len=20, explore_cnt=100, extra_steps = 5):
    total_calls = 0
    init_seed = BeamState(cur_idx_in_distb=0, prob_distrib=[1., 0, 0, 0, 0], token_id_distb=[
        eos_token_id, pad_token_id, pad_token_id, pad_token_id, pad_token_id])
    rt = []
    # construct init teacher, teacher is the pointer to the end of seq
    ...
    recombine_outputs = []
    explored_cnt = 0
    unique_explored_cnt= 0 
    while h:
        s = heapq.heappop(h)
        explored_cnt += 1
        score, seed, teacher_seed, last_state = s
        flag, rollout_output, ncall = rollout_recombine(doc_input_ids,model, seed,teacher_seed,last_state, extra_steps,max_len)
        total_calls+= ncall
        ...
        if flag:
            ...
            unique_explored_cnt += 1
            roll_out_state, fb_teacher, fb_student = rollout_output
            final_roll_out_state, rest_fb_teacher, rest_fb_student, ncall = greedy_generate_sequence(doc_input_ids, model, start_seed=roll_out_state, device=device, extra_steps=-1, max_len=max_len)
            total_calls += ncall
        ...
        if unique_explored_cnt >= explore_cnt:
            break
    # UIDs of recombs
    recom_uids = [ x.uid for x in recombine_outputs]
    outputs = [x for x in outputs if x.uid not in recom_uids]
    logging.info(f"UNIQUE / ALL: {unique_explored_cnt} {explored_cnt}")
    outputs = filter_by_score(outputs, explore_cnt)
    recombine_outputs = filter_by_score(recombine_outputs,explore_cnt)

    # combined 
    # combined_outputs = filter_by_score( outputs + recombine_outputs, explore_cnt)
    logging.info(f"==================\n\n\n\n\n\n")
    for unit in outputs:
        logging.info(repr(unit))
        rt.append(unit.get_output_str())
    stat = {
        'call':total_calls,
        'cnt_unique':unique_explored_cnt,
        'cnt':explored_cnt
    }
    return rt, stat
