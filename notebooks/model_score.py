...
for f in files:
    ...
    idx_high_model = [idx for idx, modelscore in enumerate(sum_scores) if modelscore == highest_score][0]
    dp = [ (r2s[idx_high_model]-r2 ,sum_scores[idx_high_model] - sscore) for r2, sscore in zip(r2s, sum_scores)]
    all_dps += dp
print(all_dps)
...
ax = sns.regplot(x=df['deltaR2'], y=df['deltaScore'],marker=".",line_kws={'color':'green'}, scatter_kws={'alpha':0.2})
ax.grid(True)
